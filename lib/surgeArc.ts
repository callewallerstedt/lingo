...
export type SaWord = {
  ...
  type: SaItemType;
  cue?: string; // optional scenario / production cue shown before the answer
...
export type SaState = {
  ...
  sessionHistory: SaSessionHistory[];
...
export type SaStep =
  ...
  | { kind: "trueFalseRush"; cards: SaCard[]; seconds: number }
...
export const SA_DEFAULT_SETTINGS: SaSettings = { listening: true, sentences: true, haptics: true };
...
export function saAdvanceCardStage(card: SaCard, at = Date.now()): SaCard {
  const stage = Math.min(SA_MAX_STAGE, Math.max(0, card.stage) + 1);
  return {
    ...card,
    stage,
    streak: 0,
...
export type SaLearningContext = {
  proficiency: "beginner" | "intermediate" | "advanced";
  introducedRatio: number;
  accuracy: number;
  known: string[];
  mastered: string[];
  struggling: string[];
...
export function saLearningContext(state: SaState): SaLearningContext {
  const cards = Object.values(state.cards);
  const introduced = cards.filter((card) => card.introduced);
  const mastered = cards.filter(saIsMastered);
  const seen = introduced.reduce((sum, card) => sum + card.seen, 0);
  const correct = introduced.reduce((sum, card) => sum + card.correct, 0);
  const averageStage = introduced.length
    ? introduced.reduce((sum, card) => sum + card.stage, 0) / introduced.length
    : 0;
  const proficiency =
    mastered.length >= 35 || averageStage >= 4
      ? "advanced"
      : introduced.length >= 18 || averageStage >= 2
        ? "intermediate"
        : "beginner";
  const weak = saWeakCards(state, 12);
  ...
  const focus = weak.length
    ? `Strengthen ${Array.from(new Set(weak.map(saCardTheme))).slice(0, 2).join(" and ")}`
    : proficiency === "advanced"
      ? "Natural production, nuance, and scenarios"
      : "High-frequency recall and useful sentence building";
  return {
    proficiency,
    introducedRatio: cards.length ? introduced.length / cards.length : 0,
    accuracy: seen ? correct / seen : 0,
    known: introduced
      .slice()
      .sort((a, b) => b.stage - a.stage || b.correct - a.correct)
      .slice(0, 60)
      .map((card) => card.text),
    mastered: mastered.slice(-40).map((card) => card.text),
    struggling: weak.map((card) => card.text),
    recentThemes,
...
export function saShouldGenerateUpcoming(state: SaState): boolean {
  const stats = saDeckStats(state);
  if (stats.total < 8 || (state.upcoming || []).length >= 10) return false;
  return stats.introduced / Math.max(1, stats.total) >= 0.8;
}

export function saPlanSession(state: SaState, at = Date.now()): SaSessionPlan {
  const stats = saDeckStats(state, at);
  const context = saLearningContext(state);
  const cards = saSelectSessionCards(state, at);
  const pieces: string[] = [];
  if (stats.due) pieces.push(`${Math.min(stats.due, cards.length)} due`);
  const fresh = cards.filter((card) => !card.introduced).length;
  if (fresh) pieces.push(`${fresh} new`);
  const sentenceCount = cards.filter((card) => card.type === "sentence").length;
  if (sentenceCount) pieces.push(`${sentenceCount} sentence${sentenceCount === 1 ? "" : "s"}`);
  if (context.struggling.length) pieces.push("weak-word review");

  const lessonSequence = ["pronouns", "present-tense", "questions", "past-tense", "adjectives"];
  const lessonTarget = Math.floor((state.profile.sessionsPlayed + 1) / 4);
...
export function saSelectSessionCards(state: SaState, at = Date.now()): SaCard[] {
  ...
  const weak = saWeakCards(state, 6).filter((c) => settingsFilter(c, state.settings));

  const picked: SaCard[] = [];
  const pickedIds = new Set<string>();
  const add = (card: SaCard) => {
    if (picked.length >= SA_MAX_SESSION_CARDS || pickedIds.has(card.id)) return;
  ...
  for (const c of weak) {
    if (picked.length >= Math.min(6, SA_MAX_SESSION_CARDS)) break;
    add(c);
  }
  const newRoom = Math.min(SA_NEW_PER_SESSION, SA_MAX_SESSION_CARDS - picked.length);
  let addedFresh = 0;
  for (const c of fresh) {
    if (addedFresh >= newRoom) break;
    const before = picked.length;
    add(c);
    if (picked.length > before) addedFresh += 1;
  }
...
function quizStepForCard(card: SaCard, pool: SaCard[], settings: SaSettings): SaStep {
  ...
  if (stage <= 1) {
    ...
    if (roll < 0.62) return { kind: "choice", card, options: buildOptions(card, pool, "translation") };
    return { kind: "typeTarget", card };
  }
  if (stage <= 3) {
    if (allowListen && roll < 0.18) return { kind: "listen", card, options: buildOptions(card, pool, "translation") };
    if (roll < 0.48) return { kind: "typeTarget", card };
    if (roll < 0.72 && allowProduction) return { kind: "produce", card, options: buildOptions(card, pool, "text") };
    return isSentence ? makeCloze(card, pool) : { kind: "type", card };
  }
  // Higher stages: production focus.
  if (isSentence) {
    if (roll < 0.58) return { kind: "typeTarget", card };
    if (roll < 0.82) return makeCloze(card, pool);
...
export function saBuildSteps(sessionCards: SaCard[], pool: SaCard[], settings: SaSettings): SaStep[] {
  ...
  if (matchable.length >= 4) {
    const group = saShuffle(matchable).slice(0, 5);
  ...
  for (const c of saShuffle(newCards)) {
    steps.push({ kind: "typeTarget", card: c });
  }
...
export function saBuildFocusSteps(sessionCards: SaCard[], pool: SaCard[], settings: SaSettings, mode: SaMode): SaStep[] {
  ...
  if (mode === "truefalse") {
    return sessionCards.length ? [{ kind: "trueFalseRush", cards: saShuffle(sessionCards), seconds: 60 }] : [];
  }
  if (mode === "cloze") {
  ...
  for (const card of saShuffle(sessionCards)) {
    switch (mode) {
      ...
      case "listen":
        ...
        break;
      case "build":
        steps.push({ kind: "typeTarget", card });
        break;
      default:
        steps.push({ kind: "choice", card, options: buildOptions(card, poolForDistractors, "translation") });
    }
...
export function saLoadState(language: string): SaState | null {
  ...
  try {
    ...
    return {
      ...
      sessionHistory: Array.isArray(parsed.sessionHistory) ? parsed.sessionHistory.slice(-20) : [],
      lastAutoGeneratedAt: typeof parsed.lastAutoGeneratedAt === "number" ? parsed.lastAutoGeneratedAt : 0,
...
export function saMergeWords(state: SaState, words: SaWord[]): SaState {
  ...
  return {
    ...
    lastAutoGeneratedAt: typeof state.lastAutoGeneratedAt === "number" ? state.lastAutoGeneratedAt : 0,
  };
}

export function saQueueUpcoming(state: SaState, words: SaWord[], at = Date.now()): SaState {
  const active = new Set(Object.values(state.cards).map((card) => saNormalizeKey(card.text)));
  const upcoming = [...(state.upcoming || [])];
  const queued = new Set(upcoming.map((word) => saNormalizeKey(word.text)));
  for (const word of words) {
    const key = saNormalizeKey(word.text);
    if (!key || active.has(key) || queued.has(key)) continue;
    queued.add(key);
    upcoming.push(word);
  }
  return { ...state, upcoming: upcoming.slice(0, 30), lastAutoGeneratedAt: at };
}

export function saPromoteUpcoming(state: SaState, count = 6): SaState {
  const promoted = (state.upcoming || []).slice(0, Math.max(0, count));
  const merged = saMergeWords(state, promoted);
  return { ...merged, upcoming: (state.upcoming || []).slice(promoted.length) };
}

export function saPromoteUpcomingWord(state: SaState, id: string): SaState {
  const word = (state.upcoming || []).find((item) => item.id === id);
  if (!word) return state;
  const merged = saMergeWords(state, [word]);
  return { ...merged, upcoming: (state.upcoming || []).filter((item) => item.id !== id) };
}

export function saRemoveUpcomingWord(state: SaState, id: string): SaState {
  return { ...state, upcoming: (state.upcoming || []).filter((item) => item.id !== id) };
}
